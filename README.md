...
**Maturity: `:implemented`.** 57 tests / 368 assertions pass; `clj-kondo`
...
Seven HARD checks, none of which a human approver can override:
...
3. **Unlicensed sell** -- the `:sell` right, read from the *committed* verification. An advisor cannot license itself.
...
Four jurisdictions are seeded, and **every `:provenance` URL was fetched
and read** in the session that seeded it:

| | generating | selling | authority |
|---|---|---|---|
| **JPN** | 発電事業: 届出 (電気事業法 27条の27①) | 小売電気事業: 登録 (2条の2) | 資源エネルギー庁 |
| **GBR** | s.6(1)(a) generation licence | s.6(1)(d) supply licence | GEMA / Ofgem |
| **USA** | no federal generation licence as such | market-based rate authority (FPA s.205, 18 CFR 35) | FERC |
| **DEU** | not gated by § 5 | EnWG § 5 Anzeige to BNetzA | Bundesnetzagentur |

A jurisdiction not in the table has **no** spec-basis, and the governor
holds anything that tries to invent one. Two limits are recorded rather
than papered over: the USA entry states that FERC reaches *wholesale*
sales and that no state retail regime was verified; and **no numeric
exemption threshold is encoded anywhere**, because none was verified.
Encoding a threshold you have not read is exactly the fabrication this
...
```bash
clojure -M:dev:test    # 57 tests, 368 assertions
...
seller comes up 50 Wh short), settles the interval, and then shows seven
HARD holds that never reach a human at all -- finishing by replaying the
public order log to re-derive the same book and the same 50.00 JPY fill.

## Architecture

`docs/adr/0001-architecture.md`. Superproject ADR: `2608030700`.

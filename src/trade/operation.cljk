(ns trade.operation
  "OperationActor -- one electricity-trade operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Electricity Trade Advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Market Conduct Governor
  ...
  auditable and checkpointed. In particular there is NO loop in which the
  actor places orders repeatedly on its own initiative: one human
  decision, one order.
  ...
  {:status :approved}}` (or :rejected). `:actuation/place-order`/
  `:actuation/settle-interval` ALWAYS reach this node when the governor
  is clean -- see `trade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [trade.tradeadvisor :as tradeadvisor]
...
(defn- commit-record [request _context proposal]
  {:effect  (:effect proposal)
   :path    (case (:effect proposal)
              :meter/set [(get-in proposal [:value :interval-id])
                          (get-in proposal [:value :participant-id])]
              :interval/place-order [(get-in proposal [:value :interval-id])]
              :interval/mark-settled [(get-in proposal [:value :interval-id])]
              [(:subject request)])
   :value   (or (:value proposal) {})
   :payload (:value proposal)})

(defn build
  "Compiles an OperationActor graph bound to `store` (any `trade.store/
  Store`).
  opts:
    :advisor      -- a `trade.tradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (tradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Electricity Trade Advisor inference (the contained intelligence
      ;; node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (tradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(tradeadvisor/trace request p)]})))

      ;; Market Conduct Governor -- independent censor (separate system
      ;; than the LLM), which also REPLAYS the public order log rather
      ;; than trusting any cached market state.
      (g/add-node :govern
